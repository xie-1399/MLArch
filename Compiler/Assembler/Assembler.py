...
def compileAssembler(input_file,out_file):
     parse = Parse( input_file , FirstRead=True,TableList={})
     symboltables = SymbolTable(parse.asmvalidcodes, TableList={}, getornot=True)

     parse = Parse(input_file, FirstRead=False, TableList=symboltables.TableList)
     codebinlists = Code(parse.asmvalidcodelists).codebinlists
     writeintofile(out_file, codebinlists)
