...
from Compiler.VMTranslator.CodeWriter import CodeWriter
from Compiler.VMTranslator.Parser import Parser
from Compiler.utils.HackCommonUntils import judgeBoot, ContentIsSame
# from Parser import *
...
def compileVm(input_file,output_file):
    parse = Parser(input_file)
    print(parse.vmcodeList)
    boot = judgeBoot(input_file)
    filename = input_file.split('/')[-1].replace(".vm", "")
    CodeWriter(output_file, parse.vmcodeList, filename, boot)
