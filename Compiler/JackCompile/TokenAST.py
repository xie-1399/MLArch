...
class JackASTTokenizer:
    ...
    def sanitize(self):
        ...
        """
        self.text = re.sub(consts.REGEX_COMMENTS, "", self.text)
    ...
    def advance(self):
        ...
        if self.has_more_tokens():
            REGEX_TO_ACTION = [
                ("""([a-zA-Z_{1}][a-zA-Z0-9_]+)(?=\()""", self._set_identifier_props),
    ...
    def _generic_set_props(self, regex, type, match):
        self.text = re.sub(regex, consts.EMPTY, self.text)
        self.token = match.group(1)
        self.type = type

    def _set_identifier_props(self, match):
        self._generic_set_props(
            consts.REGEX_IDENTIFIER,
            consts.IDENTIFIER,
            match
        )
